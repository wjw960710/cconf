...
以 token 認證的方式 `git pull` 本專案，並回報是否有 scripts 變動，作為 `update.ts` 的前置步驟。
...
- 比對 pull 前後 HEAD：
  ...
  - 有 commit 但變更檔案皆不在 `scripts/` 或 `plugins/*/scripts/` → exit `EXIT_NO_SCRIPTS_CHANGE` (11)。
  - 有 scripts 變動 → exit 0 並列出變動清單。
